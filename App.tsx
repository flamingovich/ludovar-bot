...
const App: React.FC = () => {
  ...
  const registerUser = async (userId: number) => {
    try {
      const res = await fetch(`${KV_REST_API_URL}/get/${USERS_LIST_KEY}`, { headers: { Authorization: `Bearer ${KV_REST_API_TOKEN}` } });
      const data = await res.json();
      let users: number[] = data.result ? JSON.parse(data.result) : [];
      if (!users.includes(userId)) {
        users.push(userId);
        await fetch(`${KV_REST_API_URL}/set/${USERS_LIST_KEY}`, { 
          method: 'POST', 
    ...
    <div className="h-screen bg-matte-black text-[#E2E2E6] overflow-hidden flex flex-col font-sans selection:bg-gold/30 relative">
      ...
      <div className="px-4 py-5 bg-soft-gray/80 backdrop-blur-lg border-b border-border-gray z-30 shadow-xl relative overflow-hidden shrink-0 shadow-gold/5">
        ...
        <div className="grid grid-cols-2 gap-3 relative z-10">
          <div className="bg-matte-black/60 backdrop-blur-sm p-3 rounded-2xl border border-border-gray/50 flex flex-col gap-1 shadow-lg shadow-black/20">
          ...
          <div className="bg-matte-black/60 backdrop-blur-sm p-3 rounded-2xl border border-border-gray/50 flex flex-col gap-1 shadow-lg shadow-gold/5">
